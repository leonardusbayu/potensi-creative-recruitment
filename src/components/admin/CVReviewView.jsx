...
export const CVReviewView = () => {
  ...
  const badge = (s, score) => {
    ...
    return <span style={{ background: c, color: "#fff", padding: "2px 8px", borderRadius: 999, fontSize: 12 }}>{s}{score != null ? ` · ${score}` : ""}</span>;
  };
  ...
  const bulkInvite = async () => {
    for (const id of selected) await inviteToInterview(id);
    showToast(`${selected.length} kandidat diundang`, "success");
    setSelected([]);
  };
  ...
  return (
    <div style={{ padding: "2rem" }}>
      ...
      <p style={{ color: "var(--text-secondary)", fontSize: 13, marginTop: 4 }}>AI skor CV (40% liveExp + 25% komunikasi + 20% availability + bonus followers). <b>HR yang memutuskan</b> — undang, tolak, atau terima kandidat.</p>
      ...
      <div style={{ marginTop: 16, background: "var(--bg-surface)", border: "1px solid var(--border-default)", borderRadius: 12, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {list.length === 0 && <tr><td colSpan={6} style={{ padding: 20, textAlign: "center", color: "var(--text-secondary)" }}>Belum ada pelamar — share apply link</td></tr>}
              {list.map((a) => (
                <tr key={a.id} style={{ borderTop: "1px solid var(--border-default)" }}>
                  ...
                  <td style={{ padding: "10px 12px" }}>
                    ...
                    <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>{a.email} · {a.wa}</div>
                    <div style={{ fontSize: 12 }}>{a.ai_summary?.slice(0, 80)}</div>
                    {a.notes && <div style={{ fontSize: 11, color: "#7c3aed", marginTop: 2 }}>📝 {a.notes.slice(0, 60)}</div>}
                  </td>
                  ...
                  <td style={{ padding: "10px 12px", display: "flex", gap: 6, flexWrap: "wrap" }}>
                    ...
                    {a.status === "pending" ? (
                      ...
                      <span style={{ fontSize: 12, color: "#7c3aed", fontWeight: 600 }}>✓ Diterima</span>
                    ) : (
      ...
      {notesFor && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: "var(--bg-surface)", borderRadius: 12, padding: 20, width: 400 }}>
            <h3 style={{ fontWeight: 700, marginBottom: 12 }}>Catatan Interview — {notesFor.name}</h3>
      ...
      {psyFor && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: "var(--bg-surface)", borderRadius: 12, padding: 20, width: 400 }}>
            <h3 style={{ fontWeight: 700, marginBottom: 12 }}>Hasil Psikotes — {psyFor.name}</h3>
